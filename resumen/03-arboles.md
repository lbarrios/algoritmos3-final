...
* Dado un grafo G son equivalentes:
	...
	* (6) **Conexo y cantidad de aristas**: `G` es conexo y `m = n − 1`.
	...
	6 => 1) Sea G un grafo conexo y con m=n−1. Quiero ver que G es un árbol. Que G es conexo se desprende trivialmente, resta ver que G no tiene circuitos simples. Por inducción en la cantidad de nodos. Caso n=1: trivial, G es un árbol. Caso inductivo: sea G=(V,X) de n nodos y n-1 aristas, quiero ver que es un árbol. Sea v∈V un nodo hoja de G, y sea T=G−v=(VT,XT) de n-1 nodos y n-2 aristas, por HI T es un árbol. Vuelvo a armar G a partir de T. Sea G'=(VT+v, XT) el grafo resultante de agregar nuevamente el nodo v a T. Dado que no existe ninguna arista que conecte v a cualquier otro nodo, G' no es conexo. En particular, G' tiene dos componentes conexas, siendo una T, y la otra v. Sea e∈G la arista tal que e∉G' (ya que el conjunto de aristas de G' es subconjunto del de G, y además este tiene 1 arista menos, luego existe esta arista e), de forma tal que G=(VT+v, XT+e). Dado que G es conexo, en particular e es una arista uniendo las dos componentes de G', por lo que es una arista puente. Luego, e no pertenece a ningún ciclo, y dado que T no tenía ciclos, G no tiene ciclos.
